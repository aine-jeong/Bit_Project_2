...
public class ThePager {
	...
	public String toString() {
		...
		// 1. 처음, 이전 항목 만들기
		if (currentPage > 1) {
			linkString.append(String.format("<a href='%s?cp=1'> << </a>", linkUrl));
			linkString.append("&nbsp;");
			linkString.append("&nbsp;");
			linkString.append(String.format("<a href='%s?cp=%d'> < </a>", linkUrl, currentPage - 1));
			linkString.append("&nbsp;");
		}
		...
		if (currentPage < pageCount) {
			...
			linkString.append("&nbsp;");
			linkString.append("&nbsp;");
			linkString.append(String.format("<a href='%s?cp=%d'> >> </a>", linkUrl, pageCount));
		}
